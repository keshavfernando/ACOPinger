...
public class Main extends ListenerAdapter {
    ...
    private static final Set<String> declineTitle = Set.of(
    ...
    private static final Set<String> notAllowed = Set.of(
            "931609777233096714",
            "484788394547740672"
    );
    ...
    // ✅ Updated: Always return non-null value
    public static String resolveUserIDs(String profile, String email, String account, DatabaseManager db) {
        String cleanedProfile = profile == null ? "" : profile.trim();
        String cleanedAccount = account == null ? "" : account.trim();
    ...
    public static String resolveItem(String product, String productOne, String description, String item) {
        if (product != null && !product.isBlank()) return product;
        else if (productOne != null && !productOne.isBlank()) return productOne;
        else if (item != null && !item.isBlank()) return item;
        else return description;
    }

    public static boolean checkDecline(String input) {
        return input != null && !input.isBlank() && declineTitle.contains(input);
    }
    ...
    // ✅ Updated: Always send webhook, even if user not found
    public static void checkWebhook(Message msg) {
        for (MessageEmbed embed : msg.getEmbeds()) {
            String title = embed.getTitle();
            String description = embed.getDescription();
            String profile = null, account = null, item = null, email = null, Product = null, productOne = null;
            ...
            String userToMention = resolveUserIDs(profile, email, account, db);
            String itemCheckedOut = resolveItem(Product, productOne, description, item);

            if (notAllowed.contains(userToMention)) {
                logger.info("User is in notAllowed list, skipping webhook.");
                continue;
            }

            if (checkDecline(title)) {
                logger.info("Detected decline webhook — sending failure notification.");
                webhook.sendCheckoutFailure(userToMention, itemCheckedOut);
            } else {
                logger.info("Detected success webhook — sending success notification.");
        ...
        for (String line : lines) {
            ...
            if (matcher.matches()) {
                ...
                if (emailPattern.matcher(email).matches()) {
                    ...
                    logger.info("Bad format detected in message line: " + line);
                }
            }
        }
    }
